...
%输入格式举例:result=jpgandalpha('girl.jpg','secret.txt')
%参数说明:
%test为原始图像
%msg为待隐藏的信息
...
for a=alpha
    resultr=resultr+1;
    [count,message,hideresult]=hidedctadv(test,'temp.jpg',msg,1999,a);
    resultc=0;
    different=0;
    for q=quality
        resultc=resultc+1;
        imwrite(hideresult,'temp.jpg','jpg','quality',q);
        msgextract=extractdctadv('temp.jpg','temp.txt',1999,count);
        for i=1:count
